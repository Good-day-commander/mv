...
#define     c_num           12 // experiment number
...
#define		XLENGTH     	ZLENGTH2 // 뒤집기
#define		YLENGTH     	YLENGTH2
#define		ZLENGTH     	XLENGTH2 // 뒤집기
...
// BC, IC
#define		uMax            0.09 // 0.01; 0.001; 0.01333; 0.03
#define		RHO_0	        1.0 // IC
...
#define		T		        200001 // simulation time (duration)
...
double		*dummy_ = new double[0]; // 주석 처리 시 코드 실행 중단 오류
//////////////////////////////// ???
...
int 	main	            (void)  
{
    ...
    sprintf(output_path, "./results/case%d_umax%.3f", c_num, uMax);
...
{   
    ...
    sprintf(FileNameVelocity, "results/case%d_umax%.3f/case_%d_data_%d.plt", c_num, uMax, c_num, Timek);    
...
void 	Log_timestep        (void)
{
    ...
    sprintf(FileName_dummy, "results/case%d_umax%.3f/case_%d_timestep.txt", c_num, uMax, c_num);
...
{   
    ...
    //#pragma omp	for private(i_nd, x, y, z)
    for (x = 1; x <= XLENGTH; x++) for (y = 1; y <= YLENGTH; y++) for (z = 1; z <= ZLENGTH; z++)
    {
    ...
    //#pragma omp	for private(i_nd, x, y, z, i, i_in)
    for (x = 1; x <= XLENGTH; x++) for (y = 1; y <= YLENGTH; y++) for (z = 1; z <= ZLENGTH; z++)
    {
        i_nd = z - 1 + ZLENGTH*(y - 1 + YLENGTH*(x - 1)) + 1;
        //*(bnode + i_nd) = 0.0;
...
void	Streaming           (void)
{
	...
	double aux, aux1, aux2, rho0, ru, u_Poi, out_pressure, yDiff, zDiff, xxx, yyy,zzz, u_p;
    ...
    #pragma omp parallel
	{
        ...
        // 중요///////////////////////////////////////////////////////////////////////
        // inlet -> outlet 바꿔야 함
        #pragma omp	for private(z, x, i, i_in, i_in2, l, h, m, u_Poi, rho0, ru,yDiff,zDiff)          
        ...
        #pragma omp	for private(z, x, i, i_in, i_in2, i_in3, i_in4, l, h, m, u_Poi, rho0, ru,yDiff,zDiff, pressure1)          
		for (y = 1; y <= YLENGTH; y++) for (z = 1; z <= ZLENGTH; z++) for (x = 1; x <= XLENGTH; x++)
		{
            i_in2 = z - 1 + ZLENGTH*(y - 1 + YLENGTH*(x - 1)) + 1;
...
void	Construct_geometry  (void)
{
    ...
    // 중요 //////////////////
	//Outlet 지정 --> inlet으로 바꿔야함 (LV - AA 연결부)
	//1 : x+ / 2: x- / 3: y- 4: z+
